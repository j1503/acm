...
void drawing::gui::render() noexcept
{
	if (this->options().active) {
		...
		ImGui::NewFrame();
		{
			cheats::cheat_manager& cm = *globals::CheatManager; // shortcut
			...
			case 5:
				...
				break;
			case 6:
				ImGui::PushID(&globals::ConfigManager->active().misc);
				{
					auto& curropt = globals::ConfigManager->active().misc;
...
config_manager::config::menu_conf& drawing::gui::options() const noexcept
{
	return globals::ConfigManager->active().general;
}